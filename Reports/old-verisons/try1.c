...
int main(){
	...
	printf("\nline id : %d",block->flag_line->id);
	printf("\nelement number is : %d ",*elements);
	
/*	for(temp =0 ; temp < block->flag_cust->size; temp += sizeof(block->flag_cust->size))
	{
		printf("\n%d . eleemnt of Customer's Y2 : %.2f'",temp+1,block->flag_cust->y2+temp);
	}
	*/
	free(block->flag_up);
